...
def _check_models():
    ...
    if not meta_path.exists():
        return {"status": "no_meta", "message": "model_meta.json not found"}
    with open(meta_path) as f:
        meta = json.load(f)
    import sklearn
    trained_version = meta.get("sklearn_version", "unknown")
    installed_version = sklearn.__version__
    ok = installed_version == trained_version
    return {
...
_health = _check_models()
if _health["status"] == "no_meta":
    st.warning("Model metadata not found. Version compatibility checks disabled.")
elif _health["status"] == "version_mismatch":
    st.error(
        f"Model files were trained with scikit-learn `{_health['trained_version']}`, "
        f"but scikit-learn `{_health['installed_version']}` is installed. "
        f"Model loading will likely fail. Install the correct version: "
        f"`pip install scikit-learn=={_health['trained_version']}`"
    )
...
def load_model(model_name):
    ...
    if meta_path.exists():
        ...
        if trained_version and trained_version != installed_version:
            st.warning(
                f"Version mismatch for **{model_name}** model: "
                f"trained with sklearn {trained_version}, "
                f"installed is {installed_version}. "
                f"Attempting to load anyway, but this may fail."
            )
    try:
...
with tab_model:
    try:
        ...
        st.error("Model Performance tab encountered an error:")
        st.exception(e)
        st.info(
            "This tab requires metric files generated by the evaluation pipeline. "
            "Run `python -m src.models.evaluate` to regenerate them."
        )
